 ...
 */
public class LTA_AntimatterFlamerTransducerEffect extends BaseCombatLayeredRenderingPlugin implements OnFireEffectPlugin, OnHitEffectPlugin, EveryFrameWeaponEffectPlugin {
    ...
    private static final int AF_EXTRA_PARTICLES_AMOUNT = 20;
	public LTA_AntimatterFlamerTransducerEffect() {
	}
	
	protected List<LTA_AntimatterFlamerTransducerEffect> trails;
	public void advance(float amount, CombatEngineAPI engine, WeaponAPI weapon) {
		if (trails == null) return;
		
		Iterator<LTA_AntimatterFlamerTransducerEffect> iter = trails.iterator();
		while (iter.hasNext()) {
		...
		if (weapon.getShip() != null) {
			...
			for (LTA_AntimatterFlamerTransducerEffect curr : trails) {
				if (curr.proj != null) {
			...
			if (weight > 0.1f) {
				...
				Global.getSoundPlayer().playLoop("LTA_AntimatterFlamer_loop", ship, 1f, volume, com, ship.getVelocity());
			}
		}
		...
		for (int i = 0; i < numIter; i++) { 
			for (LTA_AntimatterFlamerTransducerEffect trail : trails) {
	...
	public void onHit(DamagingProjectileAPI projectile, CombatEntityAPI target, Vector2f point, boolean shieldHit, ApplyDamageResultAPI damageResult, CombatEngineAPI engine) {
            ...
            ////OTHERFROMOTHERBITOFLTACODE
                    ...
                    engine.spawnExplosion(point, ZERO, AF_PARTICLE_COLOUR, 100f, 0.33f);
                    engine.spawnExplosion(point, ZERO, AF_EXPLOSION_CORE_COLOR, 50f, 0.33f);

                    DamagingExplosionSpec blast = new DamagingExplosionSpec(0.025f, 100f, 70f, MAX_DAMAGE, MIN_DAMAGE, CollisionClass.PROJECTILE_FF, CollisionClass.PROJECTILE_FIGHTER, 5f, 5f, 0.5f, AF_EXTRA_PARTICLES_AMOUNT, AF_EXPLOSION_COLOUR, null);
                    ...
                    engine.addSmoothParticle(point, ZERO, 125f, 0.1f, 0.1f, AF_PARTICLE_COLOUR);
                    engine.addHitParticle(point, ZERO, 125f, 0.3f, 0.25f, AF_EXPLOSION_FLASH_COLOR);
                    for (int x = 0; x < AF_PARTICLES_AMOUNT; x++) 
                    {
                        engine.addHitParticle(point, MathUtils.getPointOnCircumference(null, MathUtils.getRandomNumberInRange(10f, 50f), (float) Math.random() * 360f), 5f, 0.5f, MathUtils.getRandomNumberInRange(0.3f, 0.6f), AF_PARTICLE_COLOUR);
                    }
                                              }
                                   }
            //OTHERFROMOTHERBITOFLTACODE
...
//								   rampUp, dur, inverted);
	...
	public void onFire(DamagingProjectileAPI projectile, WeaponAPI weapon, CombatEngineAPI engine) {
		...
		LTA_AntimatterFlamerTransducerEffect trail = new LTA_AntimatterFlamerTransducerEffect(projectile, prev);
		...
		if (trails == null) {
			trails = new ArrayList<LTA_AntimatterFlamerTransducerEffect>();
		}
		trails.add(0, trail);
	}
...
//			offset.y += vel.y * 1f;
	...
	public LTA_AntimatterFlamerTransducerEffect(DamagingProjectileAPI proj, DamagingProjectileAPI prev) {
		this.proj = proj;
		this.prev = prev;
