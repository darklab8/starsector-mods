...
public class draco_aegis extends BaseHullMod {
	...
	public String getDescriptionParam(int index, HullSize hullSize) {
        if (index == 0) return "" + "EMP arcs to targets within 900 s.u., with the rate of arc emission increasing with flux level";
        if (index == 1) return "" + "EMP arcs are emitted much faster and with greater power";
        if (index == 2) return "" + "NOT";
		return null;
	}
...
//        IntervalUtil zapInterval = new IntervalUtil((EMP_arc_speed_level * 0.6f), (EMP_arc_speed_level * 0.8f));
        ...
        float actualLightningRange = 900f;
        ...
        if (!ship.getFluxTracker().isOverloaded() || !ship.isHulk());
        {
            ...
            if (zapInterval.intervalElapsed()) {
                ...
                if (!vents.isEmpty()) {
                    ...
                    //Then, find all valid targets: we can only shoot missiles, ships and asteroids
                    ...
                    for (CombatEntityAPI entityToTest : CombatUtils.getEntitiesWithinRange(sourcePoint, actualLightningRange)) {
                        if (entityToTest instanceof ShipAPI || entityToTest instanceof AsteroidAPI || entityToTest instanceof MissileAPI && entityToTest.getOwner() != entityToTest.getOwner()) {
