...
//		fleet.setFaction(Factions.INDEPENDENT, true);
	...
	public SectorEntityToken getMaxwellToken(Random random) {
		...
		if (pickSys == null) { Global.getLogger(this.getClass()).error("MAXWELL:CANT FIND MAXWELL"); return null; }
		...
		if (to == null || to.getContainingLocation().hasTag(Tags.SYSTEM_CUT_OFF_FROM_HYPER)) { Global.getLogger(this.getClass()).error("MAXWELL: MAXWELL INACCESSIBLE WHAT"); return null; }
		return to;
	}
	...
	public static String getTargetMaxwell(Random random) {
		...
		for(StarSystemAPI system : Global.getSector().getStarSystems()) {
			if (system.getStar() != null && (system.hasTag("HMI_MAXWELL_PICK"))) {
