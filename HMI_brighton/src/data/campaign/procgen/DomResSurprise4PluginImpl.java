...
public class DomResSurprise4PluginImpl extends BaseGenericPlugin implements SalvageDefenderModificationPlugin {
	...
	public void modifyFleet(SDMParams p, CampaignFleetAPI fleet, Random random, boolean withOverride) {
		...
		FleetMemberAPI member = fleet.getFleetData().addFleetMember("domres_eagle_std");
		...
		addAutomated(fleet, "domres_hammerhead_std", null, Commodities.BETA_CORE, random);
		addAutomated(fleet, "domres_hammerhead_cs", null, Commodities.GAMMA_CORE, random);

		addAutomated(fleet, "domres_centurion_cs", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_centurion_cs", null, Commodities.GAMMA_CORE, random);
...
//		person.getStats().setSkillLevel(Skills.COMBAT_ENDURANCE, 2);
	...
	public int getHandlingPriority(Object params) {
		...
		if (p.entity != null && p.entity.getMemoryWithoutUpdate().contains(
				HMI_manchester.DOMRES_SURPRISE4_KEY)) {
			return 2;
		}
		return -1;
	}
