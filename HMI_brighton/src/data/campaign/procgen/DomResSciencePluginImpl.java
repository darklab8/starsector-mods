...
public class DomResSciencePluginImpl extends BaseGenericPlugin implements SalvageDefenderModificationPlugin {
	...
	public void modifyFleet(SDMParams p, CampaignFleetAPI fleet, Random random, boolean withOverride) {
		...
		FleetMemberAPI member = fleet.getFleetData().addFleetMember("domres_hmi_obj_361_std");
		...
		addAutomated(fleet, "domres_hmi_obj_361_shield", null, Commodities.ALPHA_CORE, random);
		addAutomated(fleet, "domres_hmi_obj_361_cs", null, Commodities.ALPHA_CORE, random);
		addAutomated(fleet, "domres_eagle_std", null, Commodities.ALPHA_CORE, random);
		addAutomated(fleet, "domres_eagle_std", null, Commodities.ALPHA_CORE, random);

		addAutomated(fleet, "domres_outworks_std", null, Commodities.BETA_CORE, random);
		addAutomated(fleet, "domres_outworks_std", null, Commodities.BETA_CORE, random);
		addAutomated(fleet, "domres_outworks_cs", null, Commodities.BETA_CORE, random);
		addAutomated(fleet, "domres_mora_attack", null, Commodities.BETA_CORE, random);
		addAutomated(fleet, "domres_mora_attack", null, Commodities.BETA_CORE, random);

		addAutomated(fleet, "domres_hammerhead_so", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_hammerhead_so", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_berserker_atttack", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_berserker_atttack", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_berserker_atttack", null, Commodities.GAMMA_CORE, random);

		addAutomated(fleet, "domres_centurion_area", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_centurion_area", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_centurion_area", null, Commodities.GAMMA_CORE, random);
...
//		person.getStats().setSkillLevel(Skills.COMBAT_ENDURANCE, 2);
	...
	public int getHandlingPriority(Object params) {
		...
		if (p.entity != null && p.entity.getMemoryWithoutUpdate().contains(
				HMI_manchester.DOMRES_SCIENCE_KEY)) {
			return 2;
		}
		return -1;
	}
