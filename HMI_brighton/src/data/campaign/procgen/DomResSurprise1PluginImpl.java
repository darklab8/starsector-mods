...
public class DomResSurprise1PluginImpl extends BaseGenericPlugin implements SalvageDefenderModificationPlugin {
	...
	public void modifyFleet(SDMParams p, CampaignFleetAPI fleet, Random random, boolean withOverride) {
		...
		FleetMemberAPI member = fleet.getFleetData().addFleetMember("domres_mora_attack");
		...
		addAutomated(fleet, "domres_embrasure_std", null, Commodities.BETA_CORE, random);
		addAutomated(fleet, "domres_embrasure_std2", null, Commodities.BETA_CORE, random);
		addAutomated(fleet, "domres_embrasure_std3", null, Commodities.BETA_CORE, random);

		addAutomated(fleet, "domres_sentry_strike", null, null, random);
		addAutomated(fleet, "domres_sentry_cs", null, null, random);
		addAutomated(fleet, "domres_sentry_bomber", null, null, random);
		addAutomated(fleet, "domres_sentry_strike", null, null, random);
		addAutomated(fleet, "domres_sentry_cs", null, null, random);
		addAutomated(fleet, "domres_sentry_bomber", null, null, random);
		addAutomated(fleet, "domres_sentry_strike", null, null, random);
...
//		person.getStats().setSkillLevel(Skills.COMBAT_ENDURANCE, 2);
	...
	public int getHandlingPriority(Object params) {
		...
		if (p.entity != null && p.entity.getMemoryWithoutUpdate().contains(
				HMI_manchester.DOMRES_SURPRISE1_KEY)) {
			return 2;
		}
		return -1;
	}
