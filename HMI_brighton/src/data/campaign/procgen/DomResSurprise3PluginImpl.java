...
public class DomResSurprise3PluginImpl extends BaseGenericPlugin implements SalvageDefenderModificationPlugin {
	...
	public void modifyFleet(SDMParams p, CampaignFleetAPI fleet, Random random, boolean withOverride) {
		...
		FleetMemberAPI member = fleet.getFleetData().addFleetMember("domres_outworks_cs");
		member.setId("drf_" + random.nextLong());
		PersonAPI person = plugin.createPerson(Commodities.BETA_CORE, fleet.getFaction().getId(), random);
		...
		addAutomated(fleet, "domres_rampart_bombardment", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_rampart_bombardment", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_rampart_bombardment", null, Commodities.GAMMA_CORE, random);

		addAutomated(fleet, "domres_barbican_attack", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_barbican_attack", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_barbican_attack", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_barbican_attack", null, Commodities.GAMMA_CORE, random);
		addAutomated(fleet, "domres_barbican_attack", null, Commodities.GAMMA_CORE, random);

		addAutomated(fleet, "domres_picket_attack", null, null, random);
		addAutomated(fleet, "domres_picket_attack2", null, null, random);
		addAutomated(fleet, "domres_picket_attack3", null, null, random);
		addAutomated(fleet, "domres_picket_attack", null, null, random);
		addAutomated(fleet, "domres_picket_attack2", null, null, random);
		addAutomated(fleet, "domres_picket_attack3", null, null, random);
		addAutomated(fleet, "domres_picket_attack", null, null, random);
...
//		person.getStats().setSkillLevel(Skills.COMBAT_ENDURANCE, 2);
	...
	public int getHandlingPriority(Object params) {
		...
		if (p.entity != null && p.entity.getMemoryWithoutUpdate().contains(
				HMI_manchester.DOMRES_SURPRISE3_KEY)) {
			return 2;
		}
		return -1;
	}
